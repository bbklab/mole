...
%changelog
* Fri Nov 28 2014 ESOP WORKGROUP <esop_workgroup@eyou.net>
- 发布: 正式版 1.2.0
- 新增: 完全重构的新版传输代理通道, 拆分为3个独立运行模块: 接收端/队列管理/发送端
- 新增: 传输代理通道可控制待发数据队列总数, 可实时查看各独立模块运行时的统计数据
- 新增: 自带插件notify_syslogin新增参数, 可以排除某些用户或某些来源IP的登陆提醒
- 新增: 自带插件notify_syslogin新增对公钥认证的系统登陆事件的提醒
- 优化: 传输代理通道功能更加稳定, 日志更清晰的跟踪数据流, 解决通道经常丢数据的问题
- 优化: 传输代理通道控制会话超时, 彻底解决通道无响应导致的插件执行超时问题
- 优化: 数据压缩和加密从传输通道转移到mole实现, 从生成的源头保护数据
- 优化: 合并system_fixinfo/system_dyninfo的插件数据为可增量变化的协议数据, 压缩传输量
- 优化: 完全去除PHP运行环境, 安装包体积从12M大幅精简为1.3M
