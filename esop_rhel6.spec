...
%changelog
* Thu Dec 18 2014 ESOP WORKGROUP <esop_workgroup@eyou.net>
- 发布: 正式版 1.2.0
- 新增: 新增可增量上报变化的协议数据, 合并了fixinfo/dyninfo的插件数据
- 新增: 新增亿邮系列产品相关信息的搜集, 新增磁盘IO和进程信息的搜集
- 新增: 自带插件notify_syslogin新增参数, 可以排除某些用户或某些来源IP的登陆提醒
- 新增: 自带插件notify_syslogin新增对公钥认证的系统登陆事件的提醒
- 优化: 完全去除PHP运行环境, 安装包体积从12M大幅精简为1.3M
- 优化: 完全重构传输代理通道(Select+Prefork), 并拆分为3个独立运行模块
- 优化: 传输代理通道功能更加稳定, 彻底解决通道经常丢数据的问题, 通道日志更清晰的跟踪数据流
- 优化: 传输代理通道控制会话超时, 彻底解决通道无响应导致的插件执行超时问题
- 优化: 可实时查看传输代理通道各独立模块运行时的统计数据, 可控制队列总数
